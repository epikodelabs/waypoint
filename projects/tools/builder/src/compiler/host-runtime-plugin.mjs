...
export const WAYPOINT_HOST_RUNTIME_GLOBAL_KEY = '__WAYPOINT_SERVER_NAVIGATION_HOST_RUNTIME_V1__';
export function isDefaultHostModule(specifier) {
 ...
 */
export async function collectHostModuleExports(root) {
    const result = new Map();
    for (const file of await listJavaScriptFiles(root)) {
        collectFromSource(await fs.readFile(file, 'utf8'), file, result);
    }
...
export function createHostRuntimePlugin(exportsByModule) {
    return {
        ...
        setup(build) {
            ...
            build.onLoad({
                ...
                contents: hostModuleSource(args.path, exportsByModule.get(args.path)
                    ?? new Set()),
                loader: 'js',
            }));
        },
...
function hostModuleSource(specifier, exportNames) {
    const lines = [
        `const runtime = globalThis[${JSON.stringify(WAYPOINT_HOST_RUNTIME_GLOBAL_KEY)}];`,
        `const module = runtime?.modules?.get(${JSON.stringify(specifier)});`,
        `if (!module) throw new Error(${JSON.stringify(`Waypoint artifact requires host module "${specifier}". Register it with createServerNavigationResolver({ hostModules: ... }).`)});`,
    ];
    for (const name of [...exportNames].sort()) {
        ...
        if (!isIdentifier(name)) {
            throw new Error(`Cannot bridge non-identifier export ${JSON.stringify(name)} from ${JSON.stringify(specifier)}.`);
        }
        lines.push(`export const ${name} = module[${JSON.stringify(name)}];`);
    }
    return `${lines.join('\n')}\n`;
}
function collectFromSource(text, fileName, result) {
    ...
    for (const statement of source.statements) {
        if (ts.isImportDeclaration(statement)
            ...
            if (bindings
                ...
                namespaceImports.set(bindings.name.text, {
                    specifier,
                    names,
                });
            }
        ...
        if (ts.isExportDeclaration(statement)
            ...
            && ts.isStringLiteral(statement.moduleSpecifier)
            && isDefaultHostModule(statement.moduleSpecifier.text)) {
            const specifier = statement.moduleSpecifier.text;
            const names = requireSet(result, specifier);
            if (!statement.exportClause
                || !ts.isNamedExports(statement.exportClause)) {
                throw new Error(`Host-shared export * is not supported in ${JSON.stringify(fileName)}.`);
            }
...
function requireSet(result, specifier) {
