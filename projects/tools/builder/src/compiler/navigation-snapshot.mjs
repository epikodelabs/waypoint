...
export async function loadNavigationSnapshot(projectRoot, entry, metadataRoot) {
    const routeFiles = await discoverRouteModules(path.join(projectRoot, 'src'), entry);
    const generatedRoot = path.join(metadataRoot, 'analysis');
    const generatedSourceRoot = path.join(generatedRoot, 'sources');
    await fs.mkdir(generatedSourceRoot, { recursive: true });
    const waypointStubFile = path.join(generatedSourceRoot, 'waypoint-stub.ts');
    await fs.writeFile(waypointStubFile, waypointStubSource(), 'utf8');
    const transformedModules = await Promise.all([entry, ...routeFiles].map((file, index) => writeTransformedRouteModule(file, path.join(generatedSourceRoot, `module-${index}.ts`), waypointStubFile)));
    ...
    const imports = transformedRoutes.map((file, index) => `import * as routeModule${index} from ${JSON.stringify(asImportPath(file))};`);
    const moduleDescriptors = routeFiles.map((file, index) => `{ sourceFile: ${JSON.stringify(file)}, exports: routeModule${index} }`);
    ...
    if (!Array.isArray(payload.rootRoutes)) {
        throw new Error(`Waypoint entry "${entry}" did not export a NavigationTree named "routes".`);
    }
    const contributions = [];
    for (const module of payload.modules ?? []) {
        if (typeof module.sourceFile !== 'string'
            || !module.exports
...
async function writeTransformedRouteModule(sourcePath, outputPath, waypointStubFile) {
    ...
    for (const statement of sourceFile.statements) {
        ...
        transformed += transformImportDeclaration(statement, outputPath, waypointStubFile);
        cursor = statement.getEnd();
    }
    transformed += sourceText.slice(cursor);
    transformed = transformed.replace(/\bimport\s*\(/g, '__waypointDynamicImport(');
    const prelude = [
...
function transformImportDeclaration(statement, outputPath, waypointStubFile) {
    ...
    if (specifier === '@epikodelabs/waypoint') {
        const relative = toRelativeImport(outputPath, waypointStubFile);
        return rewriteImportSpecifier(clause, relative);
    }
...
function rewriteImportSpecifier(clause, specifier) {
    ...
    if (clause.namedBindings
        ...
        parts.push(`{ ${clause.namedBindings.elements
...
function stubImportBindings(clause) {
    const statements = [];
    if (clause.name) {
        statements.push(`const ${clause.name.text} = __waypointStubValue;`);
    }
    if (clause.namedBindings
        && ts.isNamespaceImport(clause.namedBindings)) {
        statements.push(`const ${clause.namedBindings.name.text} = __waypointStubValue;`);
    }
    else if (clause.namedBindings
        && ts.isNamedImports(clause.namedBindings)) {
        for (const element of clause.namedBindings.elements) {
            statements.push(`const ${element.name.text} = __waypointStubValue;`);
        }
    }
