 ...
 */
export async function assertNoRouteArtifactKeysInHost(publicRoot, artifactKeys) {
    ...
    for (const file of files) {
        ...
        for (const artifactKey of keys) {
            if (!contents.includes(artifactKey))
                continue;
            throw new Error(`Server-delivered route artifact "${artifactKey}" leaked into public host output: ${file}. ` +
                'Remove static routesFor() imports from the Angular host configuration.');
        }
    }
}
async function readableHostFiles(root) {
    const files = [];
